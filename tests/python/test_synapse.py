...
class TestProjectLayer:
    def test_project_with_stack(self):
        layer = ProjectLayer()
        result = layer.compute(PromptContext(project_name="rockport", project_stack="laravel"))
        assert "project:rockport" in result.tag
...
class TestSynapseIntegration:
    def test_full_context_injection(self):
        ...
        ctx = PromptContext(
            ...
            cwd="/Users/dev/projects/rockport",
            git_branch="feature/auth",
            project_name="rockport",
        ...
        assert "project:rockport" in result.context_string
